...
*  modified KVT
          pricediff(i)       "price difference driving the share parameters"
...
*
 ...
 pricediff(i) $ delta.range(i)  ..  delta(i) =e= b(i) * [(p(i) / pindex) / ( pnull(i)/pindex_null)]
...
*
*   ---  Calibrate a standard CES to the expected point
*        (CES version 1, i.e. without gamma term)
*        This calculates (1) expected price index and (2) deltas that we need to achive with the pricediff equation
*
...
*        -  we keep utility at the prevoius level (stay on the same isoquant)
 ...
 u.fx        = calib_results("utility"," ","version1","observed");;

*  initialize
 delta.l (i)   = calib_results("delta",i,"version1","observed");
 delta.lo(i)   = eps;
 delta.up(i)   = 10.0 * delta.l(i);

 calib1.solprint  = 1;
 solve calib1 using CNS;
* automatic calibration test
 if(calib1.suminfes gt 1.E-4, abort "price index not recovered", calib_results);

  calib_results("gamma", " ", "version1","expected")    = 0;
  calib_results("delta", i  , "version1","expected")    = delta.l(i);
  calib_results("utility"," ","version1","expected")    = u.l;
  calib_results("pindex", " ","version1","expected")    = pindex.l;
  calib_results("demand", i  ,"version1","expected")    = x.l(i);
  calib_results("price", i   ,"version1","expected")    = p(i);


*
*   --- Calibration of the linear price difference function (modified KVT approach)
*
  b(i) = calib_results("delta", i,   "version1","observed");

  a(i) =  [calib_results("delta", i  , "version1","expected")
           - ( b(i) * ((p(i)/pindex.l) / (pnull(i)/pindex_null)))]
           /
            [(p(i)/pindex.l) / (pnull(i)/pindex_null) - 1];

 display "Parameters of the pricediff function: ", a,b;

*
*
*   --- check if we are at the same isoquant
*
 model isoquant_modKVT / ces1, pricediff/;
  u.lo        = 0;
  u.up        = +inf;
  x.fx(i)     = x.l(i);
  pindex.fx   = pindex.l;
...
model mod_KVT /index1, demand1, pricediff/;

 delta.l (i)   = calib_results("delta", i,   "version1","observed");
 delta.lo(i)   = eps;
 delta.up(i)   = 10 * delta.l(i);
...
*
 delta.l (i)   = calib_results("delta", i,   "version1","expected");
 delta.lo(i)   = eps;
 delta.up(i)   = 10 * delta.l(i);

 gamma.fx      = calib_results("gamma", " ", "version1","expected");

 x.l (i)       = calib_results("demand", i,   "version1","expected");
 x.lo(i)       = eps;
 x.up(i)       = 10 * x.l(i);

 u.fx          = u.l;

 p(i)          = calib_results("price", i,   "version1","expected");

 pindex.l      = calib_results("pindex"," ", "version1","expected");
 pindex.lo     = eps;
 pindex.up     = 10 * pindex.l;
...
*   'set key off'/
   ...
   'set style line 3 lc rgb "blue" pt 7'/

   'plot "plot_modKVT.dat" using 1:2 title "indifference curve" with lines, \' /
